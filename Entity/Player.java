...
public class Player extends Entity {
	...
	public static final int GUNSLINGER_BASE_HP = 500, GUNSLINGER_BASE_MANA = 50, GUNSLINGER_BASE_HP_GROWTH = 60,
			GUNSLINGER_BASE_MANA_GROWTH = 10, ARCHER_BASE_HP = 350, ARCHER_BASE_MANA = 60, ARCHER_BASE_HP_GROWTH = 50,
			ARCHER_BASE_MANA_GROWTH = 12, WIZARD_BASE_HP = 400, WIZARD_BASE_MANA = 150, WIZARD_BASE_HP_GROWTH = 60,
			WIZARD_BASE_MANA_GROWTH = 30, WARLOCK_BASE_HP = 425, WARLOCK_BASE_MANA = 120, WARLOCK_BASE_HP_GROWTH = 55,
			WARLOCK_BASE_MANA_GROWTH = 24;

	public static final double BASE_EXP_REQ = 3.5, HEALTH_REGEN_PER_SECOND = 1.0, MANA_REGEN_PER_SECOND = 3.0,
	...
	public static double getExpReq(int level) {
		return Math.pow(Player.BASE_EXP_REQ, 1.2 * level);
	}
	...
	public void keyPress(char c, int x, int y, Level l) {
		...
		if (basic.getKey() == c) {
			if (basic.getManaCost() < this.getMana() && basic.getCount() == 0) {
				l.addEntity(basic.onUse(l).getProjectile(x, y, l));
				this.decreaseMana(basic.getManaCost());
			}
		}
		if (c == 'e') {
			inven = !inven;
	...
	public static BufferedImage getInventoryImage(double x, double y, Item[] items, Item[] equipped, int xS, int yS,
		...
		g.setFont(new Font(Font.SERIF, Font.ITALIC,
				(int) Math.round(((y / (2 * yS)) * Toolkit.getDefaultToolkit().getScreenResolution() / 72.0))));
	...
	public void mouseRelease(int x, int y, Level l) {
		if (this.inven && select != null) {
			...
			select = null;
				...
				&& ((y - (int) 8.5 * l.getHeight() / 10) < l.getHeight() / 10)
				&& ((y - (int) 8.5 * l.getHeight() / 10) > 0)) {
			skills.get((((x - (int) (1.5 * l.getWidth() / 4)) / (l.getHeight() / 10)))).setDisplay(false);
		}
	}
